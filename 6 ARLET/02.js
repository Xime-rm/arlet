const cuentas = [
  ...
  function retirar(cuenta, monto) {
    if (cuenta.saldo >= monto) {
      cuenta.saldo -= monto;
      return `Retiro exitoso. el saldo restante es de: $${cuenta.saldo}`;
    } else {
  ...
  function depositar(cuenta, monto) {
    cuenta.saldo += monto;
    return `Depósito exitoso. Saldo actual es de: $${cuenta.saldo}`;
  }
  ...
  function transferir(origen, destino, monto) {
    if (origen.saldo >= monto) {
      origen.saldo -= monto;
      destino.saldo += monto;
      return `Transferencia exitosa. Saldo de actual es de: $${actual.saldo}, Saldo de destino: $${destino.saldo}`;
    } else {
